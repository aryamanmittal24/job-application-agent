...
function TailorResumeModal({ job, onClose }: { job: Job; onClose: () => void }) {
  ...
  async function save() { setSaving(true); try { await api(`/jobs/${job.id}/tailored-resume`, { method: "PUT", body: JSON.stringify({ keywords, draft }) }); setMessage("Saved on this Mac in JobPilot’s job-specific draft library."); } catch (error) { setMessage(error instanceof Error ? error.message : "Could not save this version"); } finally { setSaving(false); } }
