              ...
              <div className="job-list job-card-list">
                {loading ? <LoadingRows /> : pagedJobs.length ? pagedJobs.map((job) => (
                  <article className="job-card" key={job.id} onClick={() => setSelected(job)}>
                    ...
                    <aside className={`match-tile qwen-match-tile ${qwenReviews[job.id] ? "qwen-complete" : qwenRunning ? "qwen-running" : "qwen-waiting"}`} aria-label={`Qwen review for ${job.title}`}>
                      {qwenReviews[job.id] ? <><strong>{reviewScore(qwenReviews[job.id])}%</strong><span>Qwen review</span><div><small>{qwenReviews[job.id].recommendation === "apply" ? "✓ Worth applying" : qwenReviews[job.id].recommendation === "skip" ? "! Consider skipping" : "• Review before applying"}</small><small>{qwenReviews[job.id].confidence} confidence</small></div></> : qwenRunning ? <><strong className="qwen-pending-score">…</strong><span>Qwen reviewing</span><div><small>{qwenProgress.done}/{qwenProgress.total} complete</small><small>Results appear here as they finish</small></div></> : <><strong className="qwen-pending-score">?</strong><span>Qwen review</span><div><small>Not reviewed yet</small><small>Run Qwen for this page</small></div></>}
                    </aside>
                  </article>
